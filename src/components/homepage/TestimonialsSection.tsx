
import React from 'react';
import { Play, ChevronLeft, ChevronRight } from 'lucide-react';
import { motion } from 'framer-motion';
import {
  Carousel,
  CarouselContent,
  CarouselItem,
  CarouselPrevious,
  CarouselNext
} from "@/components/ui/carousel";

const TestimonialsSection = () => {
  const fadeIn = {
    hidden: { opacity: 0, y: 20 },
    visible: { 
      opacity: 1, 
      y: 0,
      transition: { duration: 0.6 }
    }
  };
  
  const staggerChildren = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.2
      }
    }
  };

  const testimonials = [
    {
      name: "Michael T.",
      role: "",
      text: "The football prediction model is incredibly accurate! I've improved my success rate by over 30% since I started using it.",
      bgColor: "from-green-600/40 to-green-600/10"
    },
    {
      name: "Jessica L.",
      role: "",
      text: "The Blackjack advisor helped me understand optimal strategy and has completely changed how I approach the game.",
      bgColor: "from-amber-600/40 to-amber-600/10"
    },
    {
      name: "David W.",
      role: "",
      text: "I love the community feature - following top predictors and seeing their analysis has been a game-changer.",
      bgColor: "from-blue-600/40 to-blue-600/10"
    },
    {
      name: "Sarah M.",
      role: "",
      text: "Using the AI predictions for basketball has completely transformed my betting strategy. Highly recommend!",
      bgColor: "from-purple-600/40 to-purple-600/10"
    },
    {
      name: "Robert K.",
      role: "",
      text: "The real-time analytics feature has been invaluable for making quick decisions during live games.",
      bgColor: "from-red-600/40 to-red-600/10"
    },
    {
      name: "Emma J.",
      role: "",
      text: "I've been using the roulette advisor for a month and my win rate has improved dramatically. Amazing tool!",
      bgColor: "from-teal-600/40 to-teal-600/10"
    },
  ];

  return (
    <section className="py-16 bg-dark">
      <div className="container px-4">
        <motion.div 
          className="text-center mb-12"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: "-100px" }}
          variants={fadeIn}
        >
          <h2 className="text-3xl font-bold mb-2">What Our Users Say</h2>
        </motion.div>
        
        <Carousel 
          opts={{
            align: "start",
            loop: true,
          }}
          className="w-full"
        >
          <CarouselContent className="-ml-2 md:-ml-4">
            {testimonials.map((testimonial, index) => (
              <CarouselItem key={index} className="pl-2 md:pl-4 lg:basis-1/3 md:basis-1/2 basis-full">
                <motion.div 
                  className="rounded-xl overflow-hidden relative h-full"
                  variants={fadeIn}
                  whileHover={{ scale: 1.02 }}
                  transition={{ duration: 0.2 }}
                >
                  <div className={`relative h-[450px] bg-gradient-to-b ${testimonial.bgColor} bg-dark-darker`}>
                    {/* Video play button */}
                    <div className="absolute left-4 top-4 w-12 h-12 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center">
                      <Play size={20} className="text-white ml-1" />
                    </div>
                    
                    {/* Text content aligned to bottom */}
                    <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 to-transparent pt-20">
                      <p className="text-white text-xl font-medium mb-4 leading-tight">
                        "{testimonial.text}"
                      </p>
                      
                      <div className="flex items-center">
                        <div className="w-10 h-10 rounded-full bg-gray-300 mr-3 flex items-center justify-center text-gray-600 font-bold">
                          {testimonial.name.charAt(0)}
                        </div>
                        <div className="text-left">
                          <p className="font-medium text-white">{testimonial.name}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </motion.div>
              </CarouselItem>
            ))}
          </CarouselContent>
          <div className="flex justify-center mt-6 gap-4">
            <CarouselPrevious className="relative static left-0 right-auto translate-y-0 h-10 w-10" />
            <CarouselNext className="relative static right-0 left-auto translate-y-0 h-10 w-10" />
          </div>
        </Carousel>
      </div>
    </section>
  );
};

export default TestimonialsSection;
